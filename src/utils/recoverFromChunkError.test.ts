...
function encenar(registros: RegistroFalso[]) {
  ...
  Object.defineProperty(window, 'location', {
    configurable: true,
    value: { href: 'https://www.japanexpress-store.com/carrinho', replace },
  });
...
describe('recoverFromChunkError', () => {
  ...
  it('derruba o SW do Workbox e PRESERVA o de push', async () => {
    const workbox = registro('https://www.japanexpress-store.com/');
    const push = registro('https://www.japanexpress-store.com/push/');
  ...
  it('limpa o Cache Storage, que é onde mora o chunk velho', async () => {
    encenar([registro('https://www.japanexpress-store.com/')]);
  ...
  it('não recarrega duas vezes seguidas — a trava contra loop continua', async () => {
    const { replace } = encenar([registro('https://www.japanexpress-store.com/')]);
